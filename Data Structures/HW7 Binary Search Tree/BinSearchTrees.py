...
class BinarySearchTree(Tree):
    ...
    def iterative_search(self, node, v):
        ...
        # def helper(cur:BinarySearchTree.TreeNode, prev:BinarySearchTree.TreeNode):
        #     while cur:
        #         if not cur:
        #             return prev
        #         if v==cur._element:
    ...
    def LCA(self, node1, node2):
        ...
        """
        # TODO: modify the code below (placeholder code)
        if not node1._element<node2._element:
            node1, node2=node2, node1
        def subtree_search(node1, node2):
            if not node1:
                return False
            if node2._element==node1._element:
                return True
            if node2._element>node1._element:
                return subtree_search(node1._right, node2)
            elif node2._element<node1._element:
                return subtree_search(node1._left, node2)
        
            
        def after_ancestor(node:BinarySearchTree.TreeNode):
            walk=node._parent
            cur=node
            while walk and walk._right==cur:
                cur=walk
                walk=walk._parent
            return walk
        
        cur=node1
        while not subtree_search(cur, node2):
            cur=after_ancestor(cur)
        return cur
    ...
    def diameter(self):
        ...
        # TODO: modify the code below (placeholder code)
        self.max_diameter=0
        def helper(node: BinarySearchTree.TreeNode):
            if not node:
                return -1
            h_left=1+helper(node._left)
            h_right=1+helper(node._right)
            temp=h_left+h_right
            if temp>self.max_diameter:
                self.max_diameter=temp
...
def main():
    ...
    # print(bst.LCA(node_13, node_3)._element, "    Expected result is 10")
