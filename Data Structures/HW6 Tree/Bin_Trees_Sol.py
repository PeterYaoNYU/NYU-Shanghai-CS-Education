...
class Tree:
    ...
    def has_duplicate(self):
        ...
        """
        # Task 1
        for target in self.preorder():
            for each in self.preorder():
                if each._element == target._element and each != target:
                    return True
        return False
...
def build_expression_tree(infix):
    ...
    pri = {'+':1, '-':1, '*':2, '/':2}
    number_stack = []  # Create a stack
    operator_stack = [] # Create a stack
    inputs = infix.split()
    for each in inputs:
        if each.isdigit():  # Operand case
            tree = Tree()
            tree.add_root(int(each))
            number_stack.append(tree)
        else:  # Operator case, pop twice
            while len(operator_stack) != 0 and pri[operator_stack[-1]] >= pri[each]:
                subtree = Tree()
                subtree.add_root(operator_stack.pop())
                right_branch = number_stack.pop()
                left_branch = number_stack.pop()
                subtree._attach(subtree._root, left_branch, right_branch)
                number_stack.append(subtree)
...
# Task 4
def zero_one_update(one_zero_tree, node = None, curdepth = 0):
    """
    ...
    """
    if node is None:
        node = one_zero_tree._root
    if one_zero_tree.is_leaf(node):
        return node._element
    f = max if curdepth % 2 == 0 else min
    node._element = f([zero_one_update(one_zero_tree, child, curdepth + 1) for child in one_zero_tree.children(node)])
    return node._element
