...
class HeapLinked(PriorityQueueBase, Tree):
    ...
    def _swap(self, x, y):
        ...
        # x_left=x._left
        # x_right=x._right
        # x_left_flag=True if x._parent._left==x else False
        
        # y_parent=y._parent
        # y_left=y._left
        # y_right=y._right
        # y_left_flag=True if y._parent._left==y else False
    ...
    def _upheap(self, x):
        ...
        if not self.is_root(x):
            while x._element<x._parent._element:
                self._swap(x, x._parent)
                x=x._parent
                if self.is_root(x):
                    break
    ...
    def _downheap(self,y):
        ...
        if not self.is_leaf(y):
            small_child=y._left
            left_flag=True
            if y._right and y._right._element<small_child._element:
                small_child=y._right
                left_flag=False
            if small_child._element<y._element:
                if left_flag:
                    self._swap(y, y._left)
                    y=y._left
                else:
    ...
    def create_next_last(self, x):
        ...
        # elif not x._parent._right:
        #     x._parent._right=new
        else:
            walk=self.after(x)
            if not walk._right:
                walk._right=new
                new._parent=walk
                return new
            if walk==self._root and self.not_found_flag:
                while walk._left:
                    walk=walk._left
                walk._left=new
                new._parent=walk
                self.not_found_flag=False
    ...
    def after(self, x):
        ...
        p=walk._parent
        while p._right==walk:
        # while p._right==walk and not self.is_root(p):
            if self.is_root(p):
    ...
    def prev_last(self, x):
        ...
        if self.is_root(x):
            ...
            walk=self.upper_before(x)
            if walk._right==x:
                return walk._left
            if self.is_root(walk) and self.not_found_flag:
                while walk._right:
                    walk=walk._right
                return walk
            else:
    ...
    def upper_before(self, x):
        self.not_found_flag=False
        walk=x
        p=walk._parent
        # while p._left==walk and not self.is_root(p):
        while p._left==walk:
            if self.is_root(p):
                self.not_found_flag=True
                break
    ...
    def add_root(self, e):
        ...
        """
        # old_root_ele=self.root._element
        # old_last=self._last
        ...
        # small_child=None
        # if self._root._left:
        #     small_child=self._root._left
        # if self._root._right and small_child._element > self._root._right._element:
        #     small_child=self._root._right
        # if small_child and small_child._element<self._root._element:
        #     self._root=small_child
        # if not self.is_leaf(self._last):
        #     self._last=self
        
        # new_last=self.create_next_last(self._last)
        # new_last._element=old_root_ele
    ...
    def add(self, k ,v):
        ...
        # pass
        # self._size+=1
        # size incremented in the create_next_last function
        new_node=self._Item(k, v)
        
        if self._size==0:
            self._size=1
            self._root=self._last=self.TreeNode(new_node)  
        else:
            last_node=self.create_next_last(self._last)
            last_node._element=new_node
            self._upheap(last_node)
    ...
    def remove_min(self):
        ...
        if self.is_empty():
            ...
            ans=self.min()
            last_node_parent=self._last._parent
            old_last=self._last
            self._last=self.prev_last(self._last)
            self._size-=1
            if last_node_parent._right==old_last:
                last_node_parent._right=None
            else:
                last_node_parent._left=None
            old_last._parent=None
            self.add_root(old_last._element)
...
class HeapThreadedTree(PriorityQueueBase, Tree):
    ...
    def add(self, k,v):
        ...
        # pass
        # if self.is_empty():
        new_node_ele=self._Item(k, v) 
        
        if self.is_empty():
            self._root=self._last=self.TreeNode(new_node_ele)
            self._size+=1
            return 
        
        self._last._next=self.TreeNode(new_node_ele)
        new_node=self._last._next
        new_node._prev=self._last
        
        if self._size==1:
            self._root._left=self._root._next=new_node
            new_node._prev=new_node._parent=self._root    
            self._size+=1
            self._last=new_node
            self._upheap(self._last)
            return 
        
        if self._last._parent._right is None:
            self._last._parent._right=new_node
            new_node._parent=self._last._parent
        else:
            new_last_parent=self._last._parent._next
            new_last_parent._left=new_node
            new_node._parent=new_last_parent
    ...
    def remove_min(self):
        ...
        self.add_root(old_last._element)
        self._size-=1
        old_last._prev._next=None
        if old_last._parent._right:
            old_last._parent._right=None
        else:
            old_last._parent._left=None
        old_last._prev=old_last._parent=None
        self._downheap(self._root)
...
class ProbeHashMap(HashMapBase):
    ...
    def _bucket_delitem(self, j, k):
        ...
        # pass
        replace_idx=s
        self._table[replace_idx]=None
        s=(s+1)%len(self._table)
        while self._table[s] is not None:
            if s!=self._hash_function(self._table[s]._key):
                self._table[replace_idx]=self._table[s]
                self._table[s]=None
                replace_idx=s
            s=(s+1)%len(self._table)
...
def main():
    ...
    for i in range(7):
        # print(i)
        # print(heap1._last._element, "last element")
        # pretty_print(heap1)
        heap1.remove_min()
